...
const AdminProducts = () => {
  const { data: items, loading, refetch } = useProducts();
  ...
  const categoryOptions = dbCategories.map(c => c.slug);
  ...
  const resetForm = () => {
    ...
    setEditing(null); setShowForm(false); setImageInput('');
  };
  ...
  const handleEdit = (p: any) => {
    ...
    setForm({
      ...
      originalPrice: p.original_price ? String(p.original_price) : '', category: p.category,
      colors: [...(p.colors || [])], sizes: [...(p.sizes || [])], inStock: p.in_stock,
      isNewArrival: p.is_new_arrival, isFastSelling: p.is_fast_selling,
      images: [...(p.images || [])],
    });
    setShowForm(true);
  };
  ...
  const handleSave = async () => {
    ...
    const payload = {
      ...
      colors: form.colors, sizes: form.sizes, in_stock: form.inStock,
      is_new_arrival: form.isNewArrival, is_fast_selling: form.isFastSelling,
    };
    if (editing) {
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    Array.from(files).forEach(file => {
      ...
      reader.onload = (ev) => {
        if (ev.target?.result) setForm(prev => ({ ...prev, images: [...prev.images, ev.target!.result as string] }));
      };
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-3">
        <input placeholder="Search products..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="flex-1 min-w-[200px] px-3 py-2 rounded-lg border border-input bg-background text-foreground text-sm focus:outline-none focus:ring-2 focus:ring-ring" />
        <select value={filterCategory} onChange={e => setFilterCategory(e.target.value)} className="px-3 py-2 rounded-lg border border-input bg-background text-foreground text-sm">
        ...
        <div className="bg-card rounded-xl p-6 shadow-warm space-y-5">
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input type="number" value={form.originalPrice} onChange={e => setForm(p => ({ ...p, originalPrice: e.target.value }))} className="w-full px-3 py-2.5 rounded-lg border border-input bg-background text-foreground text-sm focus:outline-none focus:ring-2 focus:ring-ring" placeholder="1499" />
            </div>
          </div>
          <div>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {colorOptions.map(color => (
                <button key={color} type="button" onClick={() => toggleArrayItem('colors', color)} className={`px-3 py-1 rounded-full text-xs font-medium transition-colors border ${form.colors.includes(color) ? 'bg-primary text-primary-foreground border-primary' : 'bg-background text-foreground border-input hover:bg-muted'}`}>{color}</button>
              ))}
            </div>
          </div>
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {sizeOptions.map(size => (
                <button key={size} type="button" onClick={() => toggleArrayItem('sizes', size)} className={`px-3 py-1 rounded-full text-xs font-medium transition-colors border ${form.sizes.includes(size) ? 'bg-primary text-primary-foreground border-primary' : 'bg-background text-foreground border-input hover:bg-muted'}`}>{size}</button>
              ))}
            </div>
          </div>
          <div>
            ...
            <div className="flex flex-wrap gap-3 mb-3">
              {form.images.map((img, idx) => (
                <div key={idx} className="relative w-20 h-24 rounded-lg overflow-hidden border border-border">
                  ...
                  <button type="button" onClick={() => removeImage(idx)} className="absolute top-0.5 right-0.5 p-0.5 rounded-full bg-destructive text-destructive-foreground"><X className="w-3 h-3" /></button>
                </div>
              ))}
            </div>
            <div className="flex gap-2 flex-wrap">
              ...
              <div className="flex gap-1 flex-1 min-w-[200px]">
                <input type="url" value={imageInput} onChange={e => setImageInput(e.target.value)} placeholder="Paste image URL..." className="flex-1 px-3 py-2 rounded-lg border border-input bg-background text-foreground text-sm focus:outline-none focus:ring-2 focus:ring-ring" onKeyDown={e => e.key === 'Enter' && (e.preventDefault(), addImageUrl())} />
                <button type="button" onClick={addImageUrl} className="px-3 py-2 rounded-lg border border-input bg-background text-foreground text-sm hover:bg-muted transition-colors"><LinkIcon className="w-4 h-4" /></button>
              </div>
            </div>
          </div>
          <div className="flex flex-wrap gap-4">
            <label className="flex items-center gap-2 text-sm text-foreground cursor-pointer"><input type="checkbox" checked={form.inStock} onChange={e => setForm(p => ({ ...p, inStock: e.target.checked }))} className="accent-primary w-4 h-4" /> In Stock</label>
            <label className="flex items-center gap-2 text-sm text-foreground cursor-pointer"><input type="checkbox" checked={form.isNewArrival} onChange={e => setForm(p => ({ ...p, isNewArrival: e.target.checked }))} className="accent-primary w-4 h-4" /> New Arrival</label>
            <label className="flex items-center gap-2 text-sm text-foreground cursor-pointer"><input type="checkbox" checked={form.isFastSelling} onChange={e => setForm(p => ({ ...p, isFastSelling: e.target.checked }))} className="accent-primary w-4 h-4" /> Fast Selling</label>
          </div>
              ...
              {loading ? (
                ...
                <tr key={p.id} className="border-b border-border last:border-0">
                  ...
                  <td className="px-4 py-3 text-foreground">
                    ...
                    {p.original_price && <span className="text-muted-foreground line-through ml-1 text-xs">₹{Number(p.original_price).toLocaleString()}</span>}
                  </td>
                  <td className="px-4 py-3"><span className={`px-2 py-0.5 rounded-full text-xs font-semibold ${p.in_stock ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>{p.in_stock ? 'In Stock' : 'Out'}</span></td>
                  <td className="px-4 py-3">
                    <div className="flex gap-1">
                      {p.is_new_arrival && <span className="px-1.5 py-0.5 rounded text-xs bg-blue-100 text-blue-700">New</span>}
                      {p.is_fast_selling && <span className="px-1.5 py-0.5 rounded text-xs bg-orange-100 text-orange-700">Fast</span>}
                    </div>
                  </td>
                  <td className="px-4 py-3">
                    <div className="flex gap-1">
