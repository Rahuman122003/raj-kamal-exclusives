...
const Shop = () => {
  ...
  const filtered = useMemo(() => {
    ...
    return result;
  }, [search, sortBy, selectedCategory, selectedSize, priceRange, filterParam]);
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-6">
        ...
        <select
          ...
          className="px-4 py-2.5 rounded-lg border border-input bg-background text-foreground text-sm"
        >
        ...
        <aside className={`w-56 shrink-0 space-y-6 ${showFilters ? 'block' : 'hidden'} sm:block`}>
          <div>
            ...
            <div className="space-y-1">
              <button onClick={() => setSelectedCategory('')} className={`block w-full text-left px-3 py-1.5 rounded text-sm transition-colors ${!selectedCategory ? 'bg-primary text-primary-foreground' : 'text-foreground hover:bg-muted'}`}>
                All
              </button>
              {categories.map(cat => (
                <button key={cat.id} onClick={() => setSelectedCategory(cat.slug)} className={`block w-full text-left px-3 py-1.5 rounded text-sm transition-colors ${selectedCategory === cat.slug ? 'bg-primary text-primary-foreground' : 'text-foreground hover:bg-muted'}`}>
                  {cat.name}
                </button>
              ))}
            </div>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-1">
              <button onClick={() => setSelectedSize('')} className={`px-2 py-1 rounded text-xs border transition-colors ${!selectedSize ? 'bg-primary text-primary-foreground border-primary' : 'border-input text-foreground hover:bg-muted'}`}>
                All
              </button>
              {allSizes.map(size => (
                <button key={size} onClick={() => setSelectedSize(size)} className={`px-2 py-1 rounded text-xs border transition-colors ${selectedSize === size ? 'bg-primary text-primary-foreground border-primary' : 'border-input text-foreground hover:bg-muted'}`}>
                  {size}
                </button>
              ))}
            </div>
